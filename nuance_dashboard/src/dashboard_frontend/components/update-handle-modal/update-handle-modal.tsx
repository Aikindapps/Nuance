...
import './update-handle-modal.scss';
...
export const UpdateHandleModal = () => {
  const modalContext = useContext(ModalContext);

  const { updateHandle } = usePostStore((state) => ({
    updateHandle: state.updateUserHandle
  }));
  ...
  return (
    <div className='update-handle-modal'>
      <IoCloseOutline
        onClick={() => {
      ...
      />
      <p className='modal-title'>Update the handle of an individual user</p>
      ...
      {isLoading ? <div>Processing...</div> : <button onClick={async ()=>{
        setIsLoading(true)
