...
const ReviewComment = () => {
    ...
    const handleSubmit = (e: any) => {
        ...
        const urlPattern = /https?:\/\/[^\/]+\/[^\/]+\/\d*-*([a-zA-Z0-9-]+)\/[^?]+\?comment=(\d+)/;
        ...
        if (match && match.length >= 3) {
            ...
            console.log("Extracted - Bucket Canister ID:", bucketCanisterId, "Comment ID:", commentId, "Is Violating:", isToggled);
            reviewComment(commentId, bucketCanisterId, isToggled);
        } else {
            console.log("Invalid comment URL");
            toast.error("Invalid URL, please use the share link from the comment or slack notification.");
        }
    };
