...
import { stripe, toStripeInterval, APPLICATION_FEE_PERCENT } from '../stripe';
 ...
 * Creates a Stripe Price for a writer and stores the priceId + usdAmountCents in the canister.
 *
 * Body: {
 *   writerId: string,
...
router.post('/', async (req: Request, res: Response) => {
  ...
  if (!writerId || !interval || !usdAmountCents || !nonce) {
    return res.status(400).json({ error: 'writerId, interval, usdAmountCents, and nonce are required' });
  }

  const amountCents = parseInt(usdAmountCents, 10);
  if (isNaN(amountCents) || amountCents < 100) {
    return res.status(400).json({ error: 'usdAmountCents must be a number >= 100 (minimum $1.00)' });
  }
  ...
  try {
    ...
    if (stripeAccountId.length === 0) {
      return res.status(400).json({ error: 'Writer has not completed Stripe onboarding yet' });
    }

    const stripeInterval = toStripeInterval(interval);

    // Prices are created on the PLATFORM account (destination charge model)
    // application_fee_percent is set at checkout session creation, not at price creation
    ...
    // Upsert the price tier in the canister
