...
actor PostCore {
  ...
  public shared ({ caller }) func save(postModel : PostSaveModel) : async Result.Result<Post, Text> {
    ...
    switch (saveReturn) {
      case (#ok(postBucketReturn)) {
        ...
        //the publishedDate value should be null AND
        //it shouldn't be draft AND
        //it shouldn't be a scheduled post
        let sendNotification = publishedDateHashMap.get(postBucketReturn.postId) == null and (not postBucketReturn.isDraft) and postModel.scheduledPublishedDate == null;
        ...
        //send the new article notification if sendNotification value is true
        if(sendNotification){
          ignore sendNewArticleNotification(postBucketReturn);
        };
  ...
  public shared ({caller}) func sendNewArticleNotification(postBucketResponse: PostBucketType) : async () {
    if(not Principal.equal(caller, Principal.fromActor(PostCore))){
      return;
    };
    let notifications = Buffer.Buffer<(Text, CanisterDeclarations.NotificationContent)>(0);
    //firstly, get all the followers from the user canister
    let alreadySentPrincipalsMap = Map.new<Text, Text>();
    let {nhash; thash; } = Map;
    let UserCanister = CanisterDeclarations.getUserCanister();
    let followers = await UserCanister.getFollowersPrincipalIdsByPrincipalId(postBucketResponse.postOwnerPrincipal);
    //build the notification and add it to the notifications buffer for all publication followers
    for(followerPrincipalId in followers.vals()){
      ...
      //the notificaiton twice if they're also storing the tag
      Map.set(alreadySentPrincipalsMap, thash, followerPrincipalId, followerPrincipalId);
    };

    //now, get the followers of the tag
    let tagModels = getTagModelsByPost(postBucketResponse.postId);
    let tagNames = Array.map(tagModels, func (tagModel: PostTagModel) : Text {
      tagModel.tagName
    });
    let tagFollowersArrays = getTagsFollowers(tagNames);
    var i = 0;
    for(tagFollowers in tagFollowersArrays.vals()){
      for(tagFollower in tagFollowers.vals()){
        switch(Map.get(alreadySentPrincipalsMap, thash, tagFollower)) {
          case(?value) {
            //already added to the notifications buffer
          };
          case(null) {
    ...
    //added all the notifications to the notifications buffer
...
//ToDo: create a new hashmap to directly hold the array of followers mapped to the tag id
private func getTagsFollowers(tagNames : [Text]) : [[Text]] {
  var result = Buffer.Buffer<[Text]>(0);
  ...
  for(tagName in tagNames.vals()){
    ...
    result.add(Buffer.toArray(followers));
  };
