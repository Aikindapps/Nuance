...
export async function getPostCoreActor(): Promise<ActorSubclass<PostCoreService>> {
  let loginMethod = useAuthStore.getState().loginMethod;
  if (loginMethod === 'bitfinity') {
    let window_any = window as any;
    return await window_any.ic.bitfinityWallet.createActor({
...
export async function getPostBucketActor(postBucketCanisterId: string): Promise<ActorSubclass<PostBucketService>> {
  let loginMethod = useAuthStore.getState().loginMethod;
  if (loginMethod === 'bitfinity') {
    let window_any = window as any;
    return await window_any.ic.bitfinityWallet.createActor({
