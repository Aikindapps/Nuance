...
actor class EXTNFT() = this {
  ...
  private stable var data_internalNextSubAccount : Nat = 1;
  ...
  private stable var config_royalty : [(AccountIdentifier, Nat64)] = [];
  //not being used
  private stable var config_initial_sale_royalty_address : AccountIdentifier = "";
  private stable var config_collection_name : Text  =  "";
  private stable var config_collection_data : Text  = "{}";
  private stable var config_marketplace_open : Time  = 0;

  //nuance additions
  private stable var postId = "";
  private stable var writer_principal_id = Principal.fromText(ENV.SNS_GOVERNANCE_CANISTER);
  private stable var metadata_for_all : ?Metadata = null;
  private stable var thumbnail_svg = "";
  private stable var initial_supply = 0;
  private stable var max_supply = 0;
  private stable var icp_price_e8s = 100_000_000;
  //TokenIndex -> token index of the available token
  //Nat -> subaccount index that holds the token
  private stable var available_selling_token_index : ?TokenIndex = null;
  private stable var available_selling_token_subaccount = U.natToSubAccount(4294967296);
  ...
  public shared(msg) func ext_marketplaceList(request: ListRequest) : async Result.Result<(), CommonError> {
    _ext_internal_marketplaceList(msg.caller, request);
  };
  ...
  sc #= "https://"#Principal.toText(a.0)#".raw.ic0.app/: " # debug_show(a.1) # "/" # debug_show(MAX_CHUNK_STORAGE) # "\n";
    ...
    return {
      ...
      body = Text.encodeUtf8 (
        ...
        "---\n" #                                          
        "Admin:                                            " # debug_show(admins) # "\n" #
        "Owner:                                            " # debug_show(admins) # "\n"
      );
      streaming_strategy = null;
      upgrade = false;
    };
  ...
  func _ext_internal_marketplaceList(caller : Principal, request: ListRequest) : Result.Result<(), CommonError> {
  ...
  public shared ({caller}) func setConfigData(initData: InitNftCanisterData) : async Result.Result<(), Text>{
    let isController = await isCallerOwner(caller);
    if(isController or _isAdmin(caller)){
      //CONFIG
      admins := initData.admins;
      config_royalty := initData.royalty;
      config_collection_name := initData.collectionName;
      config_marketplace_open := initData.marketplaceOpen;
      thumbnail_svg := initData.thumbnail;
      metadata_for_all := ?initData.metadata;
      max_supply := initData.maxSupply;
      initial_supply := initData.initialMintingAddresses.size();
      icp_price_e8s := initData.icpPrice;
      postId := initData.postId;
      writer_principal_id := initData.writerPrincipal;

      //mint nfts to the given  initial addresses
      let mint_result = ext_mint_helper_private(initData.initialMintingAddresses);
      //mint first token available to sell
      mint_and_list_next()
    }
    else{
      return #err("Unauthorized to pass config data.")
    }
  };

  public shared(msg) func ext_mint_helper(request : [(AccountIdentifier)]) : async [TokenIndex] {
    assert(_isAdmin(msg.caller));
    ext_mint_helper_private(request)
  };

  private func ext_mint_helper_private(request : [(AccountIdentifier)]) : [TokenIndex] {
    switch(metadata_for_all) {
      case(?metadata) {
        var ret : [TokenIndex] = [];
        for(r in request.vals()){
          _tokenMetadata.put(data_internalNextTokenId, metadata);
          _transferTokenToUser(data_internalNextTokenId, r);
          data_supply := data_supply + 1;
          ret := Array.append(ret, [data_internalNextTokenId]);
          data_internalNextTokenId := data_internalNextTokenId + 1;
        };
  ...
  private func mint_and_list_next() : Result.Result<(), Text>{
    switch(available_selling_token_index) {
      case(?available_selling_token_index_value) {
        //already have an available token
        //check if the value is still valid
        switch (_registry.get(available_selling_token_index_value)) {
          case (?token_owner) {
				    if (AID.equal(AID.fromPrincipal(Principal.fromActor(this), ?available_selling_token_subaccount), token_owner) == true) {
              //still owned by the subaccount setted
              //nothing to do
					    return #ok();
				    } else {
              //owner is different
              //set the value null, call the function again
              available_selling_token_index := null;
					    return mint_and_list_next();
				    };
          };
          case (null) {
            //not possible
      ...
      case(null) {
        //there's no available token
        //check if max_supply is enough
        if(max_supply <= data_supply){
          return #err("Out of supply to mint new. Go to the market.")
        };
        
        //mint new and list it
        let mint_result =  ext_mint_helper_private([AID.fromPrincipal(Principal.fromActor(this), ?available_selling_token_subaccount)]);
        //minted the token
        //list now
        let listing_result = _ext_internal_marketplaceList(Principal.fromActor(this), {
          from_subaccount = ?available_selling_token_subaccount;
          price = ?Nat64.fromNat(icp_price_e8s);
          token = ExtCore.TokenIdentifier.fromPrincipal(Principal.fromActor(this), mint_result[0])
        });

        switch(listing_result) {
          case(#ok(listingValue)) {
            //minted and listed successfully
            //update the local state and return
            available_selling_token_index := ?mint_result[0];
            return #ok()
          };
          case(#err(error)) {
            //not possible
            return #err("Listing failed. Try again.")
          };
        };
      };
    };
  ...
  type PremiumArticleSellingInformation = {
    ...
    sellerAccount: AccountIdentifier;
    tokenSenderAccounts: [AccountIdentifier];
    maxSupply : Nat;
    currentSupply: Nat;
    price: Nat;
    postId: Text;
  ...
  public shared query func getAvailableToken() : async PremiumArticleSellingInformation {
    {
      availableTokenIndex = available_selling_token_index;
      sellerAccount = AID.fromPrincipal(Principal.fromActor(this), ?available_selling_token_subaccount);
      tokenSenderAccounts = Buffer.toArray(token_sender_accounts_buffer);
      maxSupply = max_supply;
      currentSupply = data_supply;
      price = icp_price_e8s;
      postId = postId;
    }
  };

  public type TransactionsAndSupply = {
  ...
  public shared query func marketplaceTransactionsAndTotalSupply() : async TransactionsAndSupply {
    {
