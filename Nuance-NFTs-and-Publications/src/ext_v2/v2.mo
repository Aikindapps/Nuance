...
actor class EXTNFT(init_owner : Principal) = this {
  ...
  type NuanceTokenTransaction = {
    seller : Text;
    price : Text;
    buyer : Text;
    time : Text;
  };
  type NuanceTokenListItem = {
    tokenInformation: NuanceTokenInformation;
    tokenTransaction: ?NuanceTokenTransaction;
    owner: ?AccountIdentifier;
  };
  type AccessKeyIndex = Text;
  type MintingRequest = {
  ...
  let ENTREPOT_SALE_FEES_ADDRESS : AccountIdentifier = "ab1cd378003aaba327f6a828ade0d6c25155f58834a936754035f00dffdb9fa3";
  let ENTREPOT_SALE_FEES_AMOUNT : Nat64 = 6000;
  let ENTREPOT_MARKETPLACE_FEES_ADDRESS : AccountIdentifier = "ab1cd378003aaba327f6a828ade0d6c25155f58834a936754035f00dffdb9fa3";
  ...
  //HELPER FUNCTIONS
  public shared query func canisterId() : async Principal {
  ...
  public shared (msg) func heartbeat_external() : async () {
    if (data_internalRunHeartbeat == true) {
      try {
        ...
        //will uncomment this when we get live on ic
  ...
  public shared ({caller})func ext_marketplaceList_multiple(requests : [ListRequest]) : async [Result.Result<(), CommonError>] {
    if (Time.now() < config_marketplace_open) {
      if (_saleEnded() == false) {
        return [#err(#Other("You can not list yet"))];
      };
    ...
    for(request in requests.vals()){
      ...
      case (?token_owner) {
        if (AID.equal(owner, token_owner) == false) {
          returnsBuffer.add(#err(#Other("Not authorized")));
        }
        else{
          switch (request.price) {
          case (?price) {
        ...
        returnsBuffer.add(#ok)
        };
        
      };
      case (_) {
          returnsBuffer.add(#err(#InvalidToken(request.token)));
        };
      };
    };
    returnsBuffer.toArray()
  };

  public query func indexToTokenId(index: TokenIndex) : async TokenIdentifier{
    ExtCore.TokenIdentifier.fromPrincipal(Principal.fromActor(this), index)
  };

  public query func tokens_ext_metadata(aid : AccountIdentifier) : async Result.Result<[(TokenIndex, ?Listing, Metadata, TokenIdentifier)], Text> {
    switch (_owners.get(aid)) {
      case (?tokens) {
        var resp = Buffer.Buffer<(TokenIndex, ?Listing, Metadata, TokenIdentifier)>(0);
        for (a in tokens.vals()) {
          resp.add(a, _tokenListing.get(a), Option.unwrap(_extGetTokenMetadata(a)), ExtCore.TokenIdentifier.fromPrincipal(Principal.fromActor(this), a));
        };
        return #ok(resp.toArray());
      };
      case (_) return #err("No tokens");
    };
  };

  public query func ext_metadata_multiple(tokens : [TokenIndex]) : async Result.Result<[Metadata], Text> {
    var resultBuffer = Buffer.Buffer<Metadata>(0);
    for(tokenind in tokens.vals()){
      switch (_extGetTokenMetadata(tokenind)) {
        case (?token_metadata) {
          resultBuffer.add(token_metadata);
        };
        case (_) {
  ...
  public query func getAccessKeyIndexByTokenIndex(tokenIndex: Text) : async AccessKeyIndex{
    getAccessKeyIndexByTokenIndexInternal(Nat32.fromNat(U.textToNat(tokenIndex)))
  };

  private func getAccessKeyIndexByTokenIndexInternal(tokenIndex: TokenIndex) : AccessKeyIndex{
    let tokenIdentifier = ExtCore.TokenIdentifier.fromPrincipal(Principal.fromActor(this), tokenIndex);
    let metadata = _ext_internalMetadata(tokenIdentifier);
    switch(metadata){
      case(#ok(metadata)){
        switch(metadata){
                ...
                switch(metadataContainer){
                  case(#data(values)){
                    for(value in values.vals()){
                      if(Text.equal(value.0, "Access key index")){
                        switch(value.1){
                          case(#text(keyIndex)){
                            return keyIndex;
                          };
  ...
  public query func getOwnersOfPost(postId: Text): async [(AccessKeyIndex, AccountIdentifier)]{
    var accountIdsBuffer = Buffer.Buffer<(AccessKeyIndex, AccountIdentifier)>(0);
    for(token in _registry.entries()){
      switch(_extGetTokenMetadata(token.0)){
        case null{};
        case(?metadata){
          switch(metadata){
            case(#nonfungible(meta)){
              if(meta.asset == postId){
                accountIdsBuffer.add((getAccessKeyIndexByTokenIndexInternal(token.0), token.1))
              }
            };
            case(_){};
          }
    ...
    return accountIdsBuffer.toArray();
  };

  public query func getUserAllowedPostIds(caller: Text) : async [Text] {
    let aid = AID.fromPrincipal(Principal.fromText(caller), null);
    switch (_owners.get(aid)) {
      case (?tokens){
        var allowedPostIdsBuffer = Buffer.Buffer<Text>(0);

        //loop through all the owned tokens to control and add the postId of owned tokens to allowedPostIdsBuffer
          ...
          switch(metadata){
            case(#ok(tokenMetadata)){
              switch(tokenMetadata){
                case(#nonfungible(tokenMetadata)){
                  if(not U.arrayContains(allowedPostIdsBuffer.toArray(), tokenMetadata.asset)){
                    allowedPostIdsBuffer.add(tokenMetadata.asset);
                  };
                };
                case(_){};
              }
        ...
        return allowedPostIdsBuffer.toArray();
