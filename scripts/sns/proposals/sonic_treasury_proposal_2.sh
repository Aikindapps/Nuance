...
quill sns --canister-ids-file ./sns_canister_ids.json --pem-file $PEM_FILE make-proposal  --proposal "(record {
        title = \"Transfer 250K NUA tokens to ICP/NUA Pool on Sonic Dex\";
        url = \"https://oc.app/community/3qzyb-ryaaa-aaaar-ateiq-cai/channel/180903126530388291372782995208461639178/\";
        summary = \"This is the second of 2 proposals which add liquidity to the ICP/NUA pool on SONIC DEX (https://data.sonic.ooo/pools/rxdbk-dyaaa-aaaaq-aabtq-cai:ryjl3-tyaaa-aaaaa-aaaba-cai)
...
\";
        action = opt variant {
            TransferSnsTreasuryFunds = record {
                ...
                memo = null;
                amount_e8s = 25_000_000_000_000 : nat64;
            }
        };
    }
)" $DEVELOPER_NEURON_ID > sonic_treasury_proposal_2.json

quill send sonic_treasury_proposal_2.json